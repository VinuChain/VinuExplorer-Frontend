...
/**
 * next-sitemap configuration — SOURCE OF TRUTH for sitemap.xml and robots.txt.
 *
 * Production path: deploy/scripts/entrypoint.sh calls sitemap_generator.sh
 * which runs `yarn next-sitemap` from this directory at container startup.
 * Runtime env vars (NEXT_PUBLIC_APP_HOST, NEXT_PUBLIC_API_HOST, etc.) must be
 * set before the container starts so both the siteUrl and apiUrl resolve
 * correctly to the per-env canonical host:
 *   mainnet: https://mainnet.vinuexplorer.org
 *   testnet: https://testnet.vinuexplorer.org
 *
 * Sitemap-index strategy: generateIndexSitemap is set to true so next-sitemap
 * automatically splits large URL lists into per-section sub-sitemaps (each
 * capped at 50,000 URLs per the sitemap protocol specification).  Static routes
 * land in sitemap-0.xml; dynamic URLs fetched from additionalPaths (recent
 * blocks, txs, addresses, tokens, verified contracts) go into subsequent
 * numbered sub-sitemaps as needed.  The index file is written to
 * public/sitemap.xml and referenced by robots.txt Sitemap: directive below.
 *
 * robots.txt is generated by generateRobotsTxt: true — DO NOT create a static
 * public/robots.txt; that file would override this generated one at build time.
 */
...
module.exports = {
  siteUrl,
  // Enable sitemap-index so large URL lists are split into 50k-URL sub-sitemaps
  // per the sitemap protocol specification.  The index is written to
  // public/sitemap.xml; sub-sitemaps are public/sitemap-0.xml, sitemap-1.xml, …
