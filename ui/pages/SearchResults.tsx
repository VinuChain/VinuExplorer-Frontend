...
const SearchResultsPageContent = () => {
  ...
  //
  // isFetching covers the window before those rows land: the main search can
  // resolve first, and until the CCTX query settles its data is undefined, so
  // the length check alone would offer a sort of a list still waiting on rows
  // and then take it away when they arrive. The query is enabled only when the
  // zetachain feature is on, so isFetching stays false everywhere else and this
  // does not withhold sorting from chains that have no CCTX rows at all.
  const isClientSortable =
    !isLoading &&
    pagination.page === 1 &&
    !data?.next_page_params &&
    !zetaChainCCTXQuery.isFetching &&
